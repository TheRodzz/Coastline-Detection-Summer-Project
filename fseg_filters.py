...
def log_filter(sgm, fsize):
    ...
    for x in range(-wins_x, wins_x + 1):
        for y in range(-wins_y, wins_y + 1):
            out[wins_y + y, wins_x + x] = -1. / (math.pi * sgm ** 4.) * (
                        1. - (x * x + y * y) / (2. * sgm * sgm)) * math.exp(-(x * x + y * y) / (2. * sgm * sgm))
...
def gabor_filter(sgm, theta):
    ...
    for x in range(-wins, wins + 1):
        for y in range(-wins, wins + 1):
            ...
            out[wins + y, wins + x] = 1 / (2 * math.pi * sgm * sgm) * math.exp(
                -.5 * ((xPrime) ** 2 + (yPrime * gamma) ** 2) / sgm ** 2) * math.cos(
                2 * math.pi * f * xPrime + phs)
    return out - np.mean(out)
...
def image_filtering(img, filter_list):
    ...
    for filter in filter_list:
        assert (filter[0] == 'log') or (filter[0] == 'gabor'), 'Undefined filter name. '
        if filter[0] == 'log':
